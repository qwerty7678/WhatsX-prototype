...
export default function MessageComposer() {
  ...
  const { uniqueNumbers, duplicatesRemoved, totalEntered } = useMemo(() => {
    const raw = contactsInput
      .split(/\s|,|\n|;/)
      .map(s => s.trim())
      .filter(Boolean)
    const uniq = Array.from(new Set(raw))
    return { uniqueNumbers: uniq, duplicatesRemoved: Math.max(0, raw.length - uniq.length), totalEntered: raw.length }
  }, [contactsInput])
    ...
    <section className="space-y-4">
      ...
      <form onSubmit={handleSend} className="space-y-3">
        ...
        <div className="text-sm text-gray-700">
          Entered: {totalEntered} · Unique: {uniqueNumbers.length} {duplicatesRemoved > 0 && (
            <span className="text-red-600">· {duplicatesRemoved} duplicates removed</span>
          )}
        </div>
